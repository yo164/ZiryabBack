...
sequenceDiagram
    ...
    FB-->>SPA: ID Token (Firebase)
    SPA->>API: POST /api/auth/login { token }
    API->>FB: verifyIdToken (Admin SDK)
    API->>DB: buscar Student|Teacher|Admin por firebaseUID
    API-->>SPA: JWT + perfil + Set-Cookie auth_token
    SPA->>SPA: guardar JWT (localStorage vía AuthStorage)
    Note over SPA,API: Peticiones siguientes: Bearer + withCredentials
    SPA->>API: GET /api/auth/me (Authorization)
    API-->>SPA: datos usuario
```
...
sequenceDiagram
    ...
    participant API as task.routes / controller / service
    participant P as Prisma
    participant DB as PostgreSQL

    C->>S: getTasksByAssignment(id)
    S->>I: HttpClient GET /api/tasks/teacher-assignment/:id
    I->>API: + Authorization Bearer
    API->>API: auth → authorize(['TEACHER',...])
    API->>P: findMany Task where idTeacherAssignment
    P->>DB: SQL
    DB-->>P: filas
    P-->>API: entidades
    API-->>S: { message, data }
...
sequenceDiagram
    ...
    participant API as GET /api/notifications/events
    participant DB as Notification

    SPA->>API: EventSource (?token=JWT)
    API->>API: auth vía query.token
    loop cada N segundos
        API->>DB: poll notificaciones no leídas
        API-->>SPA: evento SSE
    end
```
